...
(function() {
  ...
  if (window.$pdk) {
      let PDK_VERSION = window.$pdk.version.major;
  ...
  function initPDKPlugin6 () {
     window.wirewax.pdkPlugin = function() {
          var VERSION = "1.0"

          function initialize(loadObj) {
              console.log("%c PDK WIREWAX plugin initialize", "background: #444; color: #ffff00");
              this.loadObj = loadObj || $pdk;
              this.controller = loadObj.controller;
          }
     ...
     window.wirewax.waxxerPlugin = new window.wirewax.pdkPlugin();
     $pdk.controller.plugInLoaded(window.wirewax.waxxerPlugin, null);
  }
  // DO NOT DELETE
  //@@-- PLUGIN
